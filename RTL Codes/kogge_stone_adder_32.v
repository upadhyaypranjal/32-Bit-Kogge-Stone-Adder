...
module kogge_stone_adder_32 #(parameter PRECISION = 32)(
    ...
    localparam NUM_STEPS = (PRECISION > 1) ? clog2(PRECISION) : 1;
        ...
        for (k = 1; k < NUM_STEPS; k = k + 1) begin : prefix_stage
            for (i = 0; i < PRECISION; i = i + 1) begin : prefix_bit
                if (i >= (1 << (k-1))) begin : update
                    assign generates[k*PRECISION + i] =
                        generates[(k-1)*PRECISION + i] |
                        (propagates[(k-1)*PRECISION + i] &
                         generates[(k-1)*PRECISION + i - (1 << (k-1))]);
                    assign propagates[k*PRECISION + i] =
                        propagates[(k-1)*PRECISION + i] &
    ...
    generate
        for (i = 0; i < PRECISION; i = i + 1) begin : final_stage
            if (i >= (1 << (NUM_STEPS-1))) begin : update
                assign generates[NUM_STEPS*PRECISION + i] =
                    generates[(NUM_STEPS-1)*PRECISION + i] |
                    (propagates[(NUM_STEPS-1)*PRECISION + i] &
                     generates[(NUM_STEPS-1)*PRECISION + i - (1 << (NUM_STEPS-1))]);
            end else begin : passthrough
                assign generates[NUM_STEPS*PRECISION + i] =
                    generates[(NUM_STEPS-1)*PRECISION + i];
            end
    ...
    assign overflow_o = (PRECISION > 1) ?
                        (generates[NUM_STEPS*PRECISION + PRECISION - 1] |
                         (propagates[PRECISION-1] &
                          generates[NUM_STEPS*PRECISION + PRECISION - 2])) :
